...
#  Provides 25, 50, and 75th percentiles
...
## Obtain min and max of the PET image using the mask created above, and output to a txt file
    ...
    echo ${SUBJECT_ID} ${MIN_AND_MAX} ${IS_PATIENT}>> ${PET_DIR}/Histograms/${REGION}_min_and_max_values_TEST.txt

## Obtain the 25, 50 (median), and 75 percentile of the PET image using the mask created above, and output to a txt file
    Percentile_25th=`fslstats r${PET_file}.nii -k ${REGION}_bi_mask.nii.gz -P 25`
    echo ${SUBJECT_ID} ${Percentile_25th} ${IS_PATIENT}>> ${PET_DIR}/Histograms/${REGION}_25th_percentile.txt

    Percentile_50th=`fslstats r${PET_file}.nii -k ${REGION}_bi_mask.nii.gz -P 50`
    echo ${SUBJECT_ID} ${Percentile_50th} ${IS_PATIENT}>> ${PET_DIR}/Histograms/${REGION}_50th_percentile.txt

    Percentile_75th=`fslstats r${PET_file}.nii -k ${REGION}_bi_mask.nii.gz -P 75`
    echo ${SUBJECT_ID} ${Percentile_75th} ${IS_PATIENT}>> ${PET_DIR}/Histograms/${REGION}_75th_percentile.txt
